...
docker run --rm \
  ...
  --mount type=bind,src=${PWD}/certs/server.crt,dst=/usr/local/apache2/conf/server.crt,readonly=true \
  --mount type=bind,src=${PWD}/certs/server.key,dst=/usr/local/apache2/conf/server.key,readonly=true \
  --publish 8080:8080 \
  --publish 8443:8443 \
  httpd:2.4
